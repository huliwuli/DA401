...
EMAcross_Close	<- daily_price_2014[]	- EMA_Close
...
```{r}
PriceChange	<- daily_price_2014[,c("Close")]	- daily_price_2014[,c("Open")]
...
stochOsc <- stoch(daily_price_2014[,c("High","Low","Close")])
...
```{r}
cbind_daily<- cbind(daily_price_2014,RSI_Close,EMAcross_Close,MACDsignal,SMI,CCI,CMO,ROC,Class,daily_ADX,obv,stochWPR)
```
```{r}
idx = which(duplicated(names(cbind_daily)))
cbind_daily = cbind_daily[,-idx]
```
...
factors_pps<-complete_2015%>%
  select(Fiscal.Period,year,Ticker,Report.Date,Return.on.Equity, Return.on.Assets,Earnings.Per.Share..Basic,Equity.Per.Share, Inflation,GDP,           ex_rate,Oil_price_quarter,CPI_Quarter,UNRATE,Adj..Close)
```
...
linear1<-lm(Adj..Close~GDP+Oil_price_quarter+ex_rate+CPI_Quarter+UNRATE+Inflation+Return.on.Assets+Return.on.Equity+Earnings.Per.Share..Basic+Equity.Per.Share,data =factors_pps_omit )
summary(linear1)
```
...
```{r}
linear2<-lm(Adj..Close~GDP+ex_rate+CPI_Quarter+UNRATE+Inflation+Return.on.Assets+Return.on.Equity+Earnings.Per.Share..Basic+Equity.Per.Share,data =factors_pps_omit )
summary(linear2)
```
```{r}
...
```{r}
linear3<-lm(Adj..Close~GDP+Oil_price_quarter+ex_rate+CPI_Quarter+UNRATE+Return.on.Assets+Return.on.Equity+Earnings.Per.Share..Basic+Equity.Per.Share,data =factors_pps_omit )
summary(linear3)
```
```{r}
...
```
    ...
    The linear 1 shows that As the adjusted R2 shows, the model explains 19.1% of the variation on
